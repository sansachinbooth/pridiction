...
try:
    from gain_predictor import EnhancedGainPredictor, run_gain_prediction_analysis, print_gain_prediction_summary
    GAIN_PREDICTOR_AVAILABLE = True
except ImportError:
    GAIN_PREDICTOR_AVAILABLE = False
    print("⚠️  Gain predictor module not available - using basic ML")
...
def train_gain_optimized_ml_predictor(df: pd.DataFrame, feature_cols: list, target_col: str = 'close', test_size: float = 0.15):
    """ Enhanced ML predictor with dedicated gain prediction """
    df = df.copy()
    ...
    # ENHANCED: Run dedicated gain prediction if available
    gain_prediction_result = None
    if GAIN_PREDICTOR_AVAILABLE and len(df) > 200:
        try:
            print_status("Running enhanced gain prediction analysis...", "info")
            gain_prediction_result = run_gain_prediction_analysis(df)
            
            if gain_prediction_result and 'current_prediction' in gain_prediction_result:
                gain_pred = gain_prediction_result['current_prediction']
                if gain_pred.get('is_gain'):
                    print_status(f"🎯 Enhanced Gain Prediction: BULLISH (Confidence: {gain_pred['confidence']:.1%})", "success")
                elif gain_pred.get('is_loss'):
                    print_status(f"🎯 Enhanced Gain Prediction: BEARISH (Confidence: {gain_pred['confidence']:.1%})", "warning")
        except Exception as e:
            print_status(f"Enhanced gain prediction failed: {e}, using standard ML", "warning")

    # Continue with standard ML training...
    scaler = RobustScaler()
    ...
    # ... rest of your existing ML training code remains the same ...

    # ENHANCED: Add gain prediction results to output
    ml_output = {
        'model': final_model,
        'scaler': scaler,
        'X_test': X_test,
        ...
        # ADD THIS LINE:
        'gain_prediction': gain_prediction_result  # Include enhanced gain prediction
    }
...
def enhanced_recommend_trade(last_price: float, predicted_price: float, atr_val: float, 
                           ...
                           trend_strength: float = 0, volume_surge: float = 1.0, gain_prediction: Dict = None) -> Dict[str, Any]:
    """
    ...
    # ENHANCED: Incorporate gain prediction if available
    gain_boost = 0.0
    if gain_prediction and 'current_prediction' in gain_prediction:
        gain_pred = gain_prediction['current_prediction']
        if gain_pred.get('is_gain') and gain_pred.get('confidence', 0) > 0.6:
            gain_boost = 0.3  # Strong bullish boost
            print_status(f"🎯 Gain Prediction: BULLISH signal (Confidence: {gain_pred['confidence']:.1%})", "success")
        elif gain_pred.get('is_loss') and gain_pred.get('confidence', 0) > 0.6:
            gain_boost = -0.3  # Strong bearish boost
            print_status(f"🎯 Gain Prediction: BEARISH signal (Confidence: {gain_pred['confidence']:.1%})", "warning")
    ...
    # APPLY GAIN BOOST to price prediction
    adjusted_diff_pct = diff_pct + (gain_boost * 100)  # Apply gain prediction influence
    
    if adjusted_diff_pct > 5.0:
        factors.append(("price_prediction", "extremely_strong_bullish", 4, prediction_weight))
    elif adjusted_diff_pct > 2.0:
        factors.append(("price_prediction", "very_strong_bullish", 3, prediction_weight))
    elif adjusted_diff_pct > 1.0:
        factors.append(("price_prediction", "strong_bullish", 2, prediction_weight))
    elif adjusted_diff_pct > GAIN_PREDICTION_CONFIG['min_gain_threshold'] * 100:
        factors.append(("price_prediction", "bullish", 1, prediction_weight * 0.9))
    elif adjusted_diff_pct < -5.0:
    ...
    # ADD GAIN PREDICTION AS SEPARATE FACTOR
    if gain_prediction and 'current_prediction' in gain_prediction:
        gain_pred = gain_prediction['current_prediction']
        gain_confidence = gain_pred.get('confidence', 0.5)
        
        if gain_pred.get('is_gain'):
            factors.append(("gain_prediction", "bullish_signal", 2, gain_confidence * 1.5))
        elif gain_pred.get('is_loss'):
            factors.append(("gain_prediction", "bearish_signal", -2, gain_confidence * 1.5)) 
...
def main():
 ...
 ################################################################################################
        ...
        print_bullet(f"Trend: {trend_color}{trend_strength_text}", Fore.WHITE)
	        ...
	        # After displaying technical indicators, add enhanced gain prediction display
        if ml_out and ml_out.get('gain_prediction'):
                         gain_pred = ml_out['gain_prediction']['current_prediction']
                         print_header("ENHANCED GAIN PREDICTION", 2)
                         print_bullet(f"Direction: {gain_pred['prediction_class']}",  Fore.GREEN if gain_pred['is_gain'] else Fore.RED if gain_pred['is_loss'] else Fore.YELLOW)
                         print_bullet(f"Confidence: {gain_pred['confidence']:.1%}", Fore.WHITE)
                         print_bullet(f"Gain Probability: {gain_pred['probabilities']['gain']:.1%}", Fore.GREEN)
                         print_bullet(f"Loss Probability: {gain_pred['probabilities']['loss']:.1%}", Fore.RED)
    ...
    last_price, predicted_next, last_atr, last_rsi, ml_confidence, timeframe,
    trend_strength, volume_surge,
    gain_prediction=ml_out.get('gain_prediction') if ml_out else None  # ADD THIS
)
        if recommendation:
        ...
        # In your main function after technical indicators:
                # Enhanced gain prediction analysis
        if gain_focus and GAIN_PREDICTOR_AVAILABLE:
            try:
                print_header("ENHANCED GAIN PREDICTION ANALYSIS", 2)
                gain_result = run_gain_prediction_analysis(df_features)
                print_gain_prediction_summary(gain_result)
                
                # Use gain prediction to enhance trading recommendation
                if gain_result['current_prediction']['is_gain']:
                    # Boost confidence for BUY signals
                    ml_confidence *= 1.3
            except Exception as e:
                print_status(f"Enhanced gain prediction failed: {e}", "warning")
