...
- Local Ollama AI (gemma3:4b) integration
...
def fetch_crypto_news(symbol, limit=5):
    """Fetch recent cryptocurrency news with sentiment analysis - FIXED API CALL"""
    try:
        # Fixed: Use proper headers and parameters for CoinGecko API
        headers = {
        ...
        # If no data, try alternative news source as fallback
        if not news_data:
            return get_fallback_news(symbol)
        
        # Filter news for relevant symbols
        relevant_news = []
        symbol_lower = symbol.lower()
        
        for news_item in news_data[:limit]:  # Limit upfront
            title = news_item.get('title', '').lower()
            description = news_item.get('description', '').lower() if news_item.get('description') else ''
            
            # Broader keyword matching for relevance
...
def get_fallback_news(symbol):
    ...
    # Simple predefined news items with varied sentiment
    fallback_news = [
        {
    ...
    # Add symbol-specific sentiment based on random factor for variety
    import random
...
def add_technical_indicators(df):
    ...
    df['rsi'] = 100 - (100 / (1 + rs))
    df['rsi'] = df['rsi'].fillna(50)  # Neutral RSI when undefined
...
class MLPredictor:
    ...
    def create_features(self, df, lookback=30):
        ...
        for i in range(lookback, len(df)-1):
            ...
            # Momentum 5 (default to 0 if NaN)
            ...
            # Volume ratio (default to 1 if NaN)
            ...
            # Target variable (percentage change to next period)
    ...
    def predict(self, df, current_price, horizon_multiplier=1):
        ...
        try:
            ...
            change = max(-0.15, min(0.15, change))  # Cap extreme predictions
            predicted_price = current_price * (1 + change)
            confidence = min(0.9, max(0.1, 0.3 + abs(change) * 2))  # Dynamic confidence
...
# -----------------------------
# Ollama AI Integration (IMPROVED)
# -----------------------------
def call_ollama(prompt, model="gemma3:4b"):
    """Improved Ollama call with better error handling and response parsing"""
    payload = {
        "model": model, 
        "prompt": prompt, 
        "stream": False, 
    ...
    try:
        ...
        if resp.status_code == 200: 
            ...
            print(f"🔍 Raw AI Response: {response_text[:200]}...")  # Debug logging
            return response_text
        else:
            print(f"❌ Ollama API error: {resp.status_code} - {resp.text}")
            return ""
    except requests.exceptions.Timeout:
        print("❌ Ollama call timeout")
        return ""
    except Exception as e:
...
def parse_ai_response(ai_response, current_price):
    ...
    if not ai_response:
        print("⚠️  No AI response received, using enhanced fallback")
    ...
    # Strategy 2: Try to extract key-value pairs from text
    try:
    ...
    # Strategy 3: Use LLM to structure the response (if long enough)
    if len(ai_response) > 100:
        try:
            structured_response = structure_unstructured_response(ai_response, current_price)
            if structured_response:
                print("✅ Successfully structured unstructured AI response")
                return structured_response
        except Exception as e:
            print(f"⚠️  Response structuring failed: {e}")
    
    # Final fallback with enhanced reasoning based on response content
    enhanced_fallback = enhance_fallback_based_on_content(ai_response, default_response, current_price)
...
def structure_unstructured_response(text, current_price):
    """Use a follow-up prompt to structure unstructured response"""
    structure_prompt = f"""
    Please analyze the following trading analysis and extract the key information in JSON format:
    
    ANALYSIS: {text[:500]}
    
    Extract as JSON with these exact keys:
    ...
    structured_response = call_ollama(structure_prompt)
    if structured_response:
        try:
            start_idx = structured_response.find('{')
            end_idx = structured_response.rfind('}') + 1
            if start_idx != -1 and end_idx != 0:
                return json.loads(structured_response[start_idx:end_idx])
        except:
            pass
