...
def add_gain_prediction_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add specialized features for gain prediction"""
    df = df.copy()
    ...
    try:
        ...
        # Volume-price correlation (strong indicator of gains)
        if 'volume' in df.columns:
            df['volume_price_corr_10'] = df['volume'].rolling(10).corr(df['close'])
            df['volume_price_corr_20'] = df['volume'].rolling(20).corr(df['close'])
            df['volume_surge_ratio'] = df['volume'] / df['volume'].rolling(20).mean()
        
        # Breakout detection
        df['resistance_breakout'] = (df['close'] > df['high'].rolling(20).max()).astype(int)
        df['support_bounce'] = (df['close'] > df['low'].rolling(20).min()).astype(int) & (df['close'].pct_change() > 0)
        
        # Trend strength with multiple timeframes
        for period in [10, 20, 50]:
            sma = df['close'].rolling(period).mean()
            price_vs_sma = (df['close'] - sma) / sma * 100
            df[f'trend_strength_{period}'] = price_vs_sma
            df[f'above_sma_{period}'] = (df['close'] > sma).astype(int)
        
        # Volatility compression/expansion (predicting big moves)
        df['volatility_ratio_5_20'] = df['close'].pct_change().rolling(5).std() / df['close'].pct_change().rolling(20).std()
        
        # Price pattern recognition
        df['higher_highs'] = (df['high'] > df['high'].shift(1)).rolling(3).sum()
        df['higher_lows'] = (df['low'] > df['low'].shift(1)).rolling(3).sum()
        
        # Market regime detection
        df['bull_market_score'] = (
            df['above_sma_20'] + 
            (df['close'] > df['close'].shift(20)).astype(int) +
            (df['rsi14'] > 50).astype(int) if 'rsi14' in df.columns else 0
        )
        ...
        # For gain-specific features, use more intelligent filling - FIXED DEPRECATION
        for col in df.columns:
            if df[col].dtype in ['float64', 'int64']:
                if 'momentum' in col or 'trend' in col:
                    df[col] = df[col].fillna(0)  # Neutral for trend/momentum
                elif 'corr' in col:
        ...
        print_status(f"Error in gain prediction features: {e}", "warning")
...
def train_gain_optimized_ml_predictor(df: pd.DataFrame, feature_cols: list, target_col: str = 'close', test_size: float = 0.15):
        """ Enhanced ML predictor optimized for gain prediction """
        df = df.copy()
    
        # Enhanced data validation
        if len(df) < 100:
          print_status("Not enough data for enhanced ML training (min 100 rows required)", "warning")
          return None
        
        # Create target variable with gain focus
        ...
        if df['target'].std() < 0.001:  # Very low volatility
          print_status("Data has very low volatility, ML prediction may be unreliable", "warning")
          return None
        ...
        low_variance_features = X.columns[X.var() < variance_threshold]
        if low_variance_features.any():
           print_status(f"Removing low variance features: {list(low_variance_features)}", "warning")
           X = X.drop(columns=low_variance_features)
           available_features = [f for f in available_features if f not in low_variance_features]
    
        if len(available_features) < 5:
            print_status("Too few features after variance filtering", "warning")
            return None
    
        # Enhanced data preprocessing for gain prediction
        scaler = RobustScaler()
        ...
        # Enhanced ensemble of models optimized for gain prediction
        # Replace the entire models dictionary with this:
        models = {
    ...
    'svr': SVR(kernel='rbf', C=1.0, epsilon=0.1),  # Added SVR instead of MLP
}
        ...
        # Train and evaluate each model with gain-focused metrics
        for name, model in models.items():
         try:
            ...
            for train_idx, val_idx in tscv.split(X_scaled):
                ...
                # Skip MLP if data is too small for this fold
                if name == 'mlp' and (len(X_train) < 50 or len(X_val) < 10):
                    print_status(f"Skipping MLP for small fold: train={len(X_train)}, val={len(X_val)}", "warning")
                    continue
                ...
                # Calculate gain prediction accuracy
                y_val_shifted = y_val.shift(1)
                y_val_shifted = y_val_shifted.bfill()
                actual_gains = (y_val > y_val_shifted).astype(int)
                predicted_gains = (preds > y_val.values).astype(int)
                gain_accuracy = (actual_gains == predicted_gains).mean()
                gain_accuracies.append(gain_accuracy)
            
            if scores:  # Only if we have valid scores
                avg_score = np.mean(scores)
                avg_gain_accuracy = np.mean(gain_accuracies) if gain_accuracies else 0.0
                model_performance[name] = {
                    'mae': avg_score,
                    'gain_accuracy': avg_gain_accuracy
                }
                ...
                print_status(f"{name.replace('_', ' ').title()} CV MAE: {avg_score:.6f}, Gain Accuracy: {avg_gain_accuracy:.2%}", "info")
            else:
                print_status(f"{name.replace('_', ' ').title()} - No valid scores", "warning")
            
         except Exception as e:
            print_status(f"Error training {name}: {str(e)}", "warning")
            continue
    
        if best_model is None:
         print_status("All models failed to train", "error")
         return None
    
        # Create ensemble model (excluding MLP if it failed)
        ensemble_models = []
        for name, model in models.items():
         if name in model_performance and name != 'mlp':  # Exclude MLP from ensemble
            perf = model_performance[name]
            weight = 1.0 + perf['gain_accuracy']
            ensemble_models.append((name, model))
    
        if len(ensemble_models) > 1:
         weights = [model_performance[name]['gain_accuracy'] + 0.5 for name, _ in ensemble_models]
         ensemble = VotingRegressor(estimators=ensemble_models, weights=weights)
         ensemble.fit(X_scaled, y)
         final_model = ensemble
         model_type = "Gain-Optimized Ensemble"
        else:
        ...
        # Enhanced gain prediction metrics
        y_test_shifted = y_test.shift(1)
        y_test_shifted = y_test_shifted.bfill()
        actual_direction = (y_test > y_test_shifted).astype(int)
        predicted_direction = (preds > y_test.values).astype(int)
        direction_accuracy = (actual_direction == predicted_direction).mean()
    
        # Gain-specific accuracy
        gain_mask = actual_direction == 1
        if gain_mask.any():
         gain_accuracy = (predicted_direction[gain_mask] == 1).mean()
        else:
         gain_accuracy = 0.0
        ...
        if hasattr(final_model, 'feature_importances_'):
         ...
         print_status(f"Top 5 features: {[f[0] for f in top_features]}", "success")
    
        print_status(f"Best model: {model_type}", "success")
        print_status(f"Final MAE: {mae:.6f}, RMSE: {rmse:.6f}, R²: {r2:.4f}", "success")
        print_status(f"Direction Accuracy: {direction_accuracy:.2%}, Gain Accuracy: {gain_accuracy:.2%}", "success")
...
GAIN_FEATURE_COLS = [
    'ema8', 'ema21', 'ema50', 'rsi14', 'rsi7', 'macd', 'macd_signal', 'macd_hist', 
    'atr14', 'bb_width', 'volume_ratio', 'ret1', 'ret3', 'ret5', 'ret7', 'ret10',
    'price_rate_of_change_5', 'price_rate_of_change_10', 'volatility_ratio', 
    'trend_strength', 'price_vs_resistance_20', 'price_vs_support_20',
    # Gain prediction specific features
    'momentum_3', 'momentum_5', 'momentum_10', 'momentum_15',
    'acceleration_3', 'acceleration_5', 'acceleration_10', 'acceleration_15',
    'volume_price_corr_10', 'volume_price_corr_20', 'volume_surge_ratio',
    'resistance_breakout', 'support_bounce', 'trend_strength_10', 'trend_strength_20', 'trend_strength_50',
    'above_sma_10', 'above_sma_20', 'above_sma_50', 'volatility_ratio_5_20',
    'higher_highs', 'higher_lows', 'bull_market_score', 'overnight_gap'
]
...
def main():
    ...
    try:
        ...
        available_features = [col for col in GAIN_FEATURE_COLS if col in df.columns]
        ...
        if not df_features.empty and len(df_features) >= 100:
            try:
                ...
                ml_out = train_gain_optimized_ml_predictor(df_features, available_features)
                
                if ml_out and ml_out.get('r2', -100) > -10:  # Basic quality check
                    if year_prediction_flag and len(df_features) >= 200:
                        print_status("Generating 1-year prediction...", "info")
                        future_predictions = enhanced_predict_future_prices(
                            ml_out['model'], ml_out.get('scaler'),
                            ml_out['last_row'], ml_out['feature_cols'], periods=365
                        )
                        if not future_predictions.empty:
                            ml_confidence = min(0.8, future_predictions['confidence'].mean())
                else:
        ...
        if ml_out and ml_out.get('model') is not None and ml_out.get('last_row') is not None:
            try:
                features_data = ml_out['last_row'][ml_out['feature_cols']].values.reshape(1, -1)
                features_df = pd.DataFrame(features_data, columns=ml_out['feature_cols'])
                
                if ml_out.get('scaler') is not None:
                    features_scaled = ml_out['scaler'].transform(features_df)
                else:
                    features_scaled = features_df.values
                    
                predicted_next = float(ml_out['model'].predict(features_scaled)[0])
